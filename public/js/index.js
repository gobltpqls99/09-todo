...
/************** 사용자함수 ***************/
function dbInit() {
	db.ref('root/todo/'+user.uid).on('child_added', onAdd);//추가
	db.ref('root/todo/'+user.uid).on('child_removed', toggleList);//삭제
	db.ref('root/todo/'+user.uid).on('child_changed', toggleList);//변경
}

function addHTML(k, v) {//key value
